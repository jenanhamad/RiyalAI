...
def register_user(username: str, password: str) -> dict:
    ...
    conn.execute(
        """INSERT INTO users (user_id, username, email, password_hash, display_name, xp, level, streak,
           ...
           VALUES (?, ?, ?, ?, ?, 0, 1, 0, 0, ?, ?, ?)""",
        (user_id, username, f"{user_id}@local.riyalai", hash_password(password), username, week_start, now, now),
    )
    conn.commit()
    conn.close()
    token = create_token(user_id, username)
    return {"token": token, "userId": user_id, "username": username, "displayName": username}
